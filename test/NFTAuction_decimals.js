...
describe("NFTAuction - 不同精度代币测试", function () {
    ...
    beforeEach(async function () {
        ...
        // 设置价格源
        await nftAuction.setPriceFeed(ethers.ZeroAddress, await mockETHUSD.getAddress());

        // 部署 DAI (18 位小数)
        const MockERC20 = await ethers.getContractFactory("MockERC20");
        mockDAI = await MockERC20.deploy("Mock DAI", "DAI", 18);
        await mockDAI.waitForDeployment();
        await nftAuction.setPriceFeed(await mockDAI.getAddress(), await mockDAIUSD.getAddress());

        // 部署 WBTC (8 位小数)
        mockWBTC = await MockERC20.deploy("Mock WBTC", "WBTC", 8);
        await mockWBTC.waitForDeployment();
        await nftAuction.setPriceFeed(await mockWBTC.getAddress(), await mockWBTCUSD.getAddress());
