...
contract NFTAuction is Initializable, ERC721HolderUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
	...
	// 事件：新的最高出价
	event NewHighestBid(uint256 indexed auctionId, address indexed bidder, uint256 amount, address token);
	 ...
	 */
	function _calculateERC20BidValue(IPriceOracleReader oracle, address token, uint256 amount) private view returns (uint256 payValueInUSD) {
		if (amount == 0) {
			revert AmountMustBeGreaterThanZero();
		}

		payValueInUSD = oracle.getTokenValueInUSD(token, amount);
	}
	 ...
	 * @return 美元价值（8位小数）
	 */
	function _convertToUSDValue(IPriceOracleReader oracle, address token, uint256 amount) private view returns (uint256) {
		// ETH
		if (token == address(0)) {
			return oracle.getEthValueInUSD(amount);
		}

		// ERC-20 代币
		return oracle.getTokenValueInUSD(token, amount);
	}
	 ...
	 * @param currentToken 当前最高出价使用的代币
	 * @param currentHighestBid 当前最高出价金额
	 */
	function _validateBidAmount(IPriceOracleReader oracle, Auction storage auction, uint256 payValueInUSD, address currentToken, uint256 currentHighestBid) private view {
		if (auction.highestBidder == address(0)) {
			...
			uint256 highestBidValueInUSD = _convertToUSDValue(oracle, currentToken, currentHighestBid);
	 ...
	 * @param previousToken 之前的代币地址
	 * @param previousBid 之前的出价金额
	 */
	function _refundPreviousBidder(address previousBidder, address previousToken, uint256 previousBid) private {
		if (previousBidder == address(0)) {
			return;
		}

		_transferOut(previousToken, previousBidder, previousBid);
	}
	...
	function placeBidETH(uint256 _auctionId) public payable nonReentrant {
		...
		_validateBidAmount(oracle, auction, payValue, auction.token, auction.highestBid);

		// 保存前一个出价者信息（用于退款）
		address previousBidder = auction.highestBidder;
		uint256 previousBid = auction.highestBid;
		address previousToken = auction.token;
		...
		// 退款给之前的出价者
		_refundPreviousBidder(previousBidder, previousToken, previousBid);
	 ...
	 * @param amount 代币数量
	 */
	function placeBidToken(uint256 _auctionId, address _token, uint256 amount) public nonReentrant {
		Auction storage auction = auctions[_auctionId];
		_validateBidConditions(auction, auction.seller);
		IPriceOracleReader oracle = _getPriceOracleReader(auction);

		uint256 payValue;  // 美元价值（8 位小数）
		payValue = _calculateERC20BidValue(oracle, _token, amount);
		...
		emit NewHighestBid(_auctionId, msg.sender, amount, _token);
	}
	...
	function endAuction(uint256 _auctionId) external nonReentrant {
		...
		if (highestBidder == address(0)) {
			...
			// 计算手续费（按策略）
			(uint256 feeAmount, address feeReceiver) = _computeFee(_auctionId, auction.seller, token, highestBid);
			uint256 sellerAmount = highestBid - feeAmount;

			// 卖家结算
			_transferOut(token, auction.seller, sellerAmount);

			// 累计手续费，等待管理员提取
			if (feeAmount > 0) {
				accruedFees[token] += feeAmount;
				emit FeeAccrued(_auctionId, token, feeReceiver, feeAmount);
			}
		}
